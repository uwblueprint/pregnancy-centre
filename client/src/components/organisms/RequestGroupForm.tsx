...
const RequestGroupForm: FunctionComponent<Props> = (props: Props) => {
  ...
  if (props.operation === "edit") {
    const { loading } = useQuery(requestGroupQuery, {
      ...
      onCompleted: (data: { requestGroup: RequestGroup }) => {
        ...
        setRequestTypesMap(retrievedRequestGroup.requestTypes ?
  ...
  const updateRequestTypesMapError = (requestTypesMap: Map<string, RequestTypeData>): string => {
    let error = "";
    if (Array.from(requestTypesMap.values())
      .filter(requestTypeData => requestTypeData.deleted === false).length === 0) {
  ...
  const onAddRequestType = (newRequestTypeName: string) => {
    ...
    if (error === "") {
      const newRequestTypeData: RequestTypeData = requestTypesMap.get(newRequestTypeName) ?? { id: null, deleted: false }
  ...
  const onDeleteRequestType = (targetRequestTypeName: string) => {
    ...
    if (requestTypeValue != null) {
      setChangeMade(true);
      setRequestTypesMap(new Map(requestTypesMap).set(targetRequestTypeName, { ...requestTypeValue, deleted: true }));
      ...
      if (props.operation === "create") {
        ...
        createRequestGroup({ variables: { name, description, image } })
          .then((response) => {
            ...
            const createRequestTypePromises = newRequestTypeNames
        ...
        const newRequestTypeNames = Array.from(requestTypesMap)
          .filter(([_requestTypeName, requestTypeData]) => requestTypeData.id == null && requestTypeData.deleted === false)
          .map(([requestTypeName, _requestTypeData]) => requestTypeName)
