...
const RequestTypeForm: FunctionComponent<Props> = (props: Props) => {
    ...
    const [requestType, setRequestType] = useState(
        props.requestType && props.requestType.name ? props.requestType.name : ""
    );
    const requestTypesSet = new Set(
        props.requestGroup.requestTypes ? props.requestGroup.requestTypes.map((requestType) => requestType.name) : []
    );
    ...
    const updateRequestTypeError = (newRequestTypeName: string) => {
        ...
        if (newRequestTypeName.length > 40) {
            ...
            (!props.requestType || newRequestTypeName !== props.requestType.name) &&
