    ...
    return (
        <div className="request-type-list">
            ...
            <ListGroup className="request-type-list" variant="flush">
                {props.requestTypes
                    .sort((requestType1 : RequestType, requestType2 : RequestType) => (requestType1.name ? requestType1.name : "") > (requestType2.name ? requestType2.name : "") ? 1 : -1)
                    .map((requestType) => <RequestTypeListItem key={requestType._id} requestType={requestType}/>)}
            </ListGroup>
        </div>
    )
}
