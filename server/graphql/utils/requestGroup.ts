...
const createRequestGroupHelper = async (requestGroup, dataSources, session): Promise<Document> => {
    ...
    if (requestGroup.requestTypeNames) {
        const requestTypePromises: Array<Promise<RequestTypeDocument>> = requestGroup.requestTypeNames.map(
            (requestTypeName) =>
...
const updateRequestGroupHelper = async (requestGroup, dataSources, session): Promise<Document> => {
    ...
    if (requestGroup.requestTypeNames) {
        ...
        const oldRequestTypes: Array<RequestTypeDocument> = await Promise.all(
            oldRequestGroup.requestTypes.map((requestTypeId) => dataSources.requestTypes.getById(requestTypeId))
        );
        ...
        const newRequestTypeNames = requestGroup.requestTypeNames.filter(
            (requestTypeName) => !oldRequestTypeNames.has(requestTypeName)
        );
        ...
        const createRequestTypePromises: Array<Promise<RequestTypeDocument>> = newRequestTypeNames.map(
            (requestTypeName) =>
                dataSources.requestTypes.create({ name: requestTypeName, requestGroup: requestGroup.id })
        );
        const newRequestTypeIds = await Promise.all(createRequestTypePromises).then(
            (requestTypes: Array<RequestTypeDocument>) => requestTypes.map((requestType) => requestType._id)
        );

        // Soft delete missing request types
        await Promise.all(
            deletedRequestTypeIds.map((requestTypeId) => softDeleteRequestTypeHelper(requestTypeId, dataSources))
        );
