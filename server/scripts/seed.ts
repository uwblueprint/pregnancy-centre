...
const createRequest = (requestID, clientIDs, requestTypeID, errMsg) => {
    ...
    const dateUpdated = isFulfilled
        ? randomDate(new Date(dateFulfilled))
        : randomDate(new Date(dateCreated));
...
connectDB(() => {
    ...
    Client.deleteMany((err) => {
        if (err) {
            console.error(
                "\x1b[31m",
                "Failed to delete all documents in 'client' collection"
            );
            console.log("\x1b[0m");
            exit();
        }
    ...
    for (let clientIdx = 0; clientIdx < numClients; clientIdx++) {
        const clientErrMsg =
            "Attempted to seed client # " + clientIdx + " but failed:";
    ...
    for (let groupIdx = 0; groupIdx < numGroups; groupIdx++) {
        ...
        for (let typeIdx = 0; typeIdx < numTypesPerGroup; typeIdx++) {
            ...
            for (
                ...
                const requestPromise = createRequest(
                    requestIDsForType[requestIdx],
                    clientIDs,
                    typeIDsForGroup[typeIdx],
                    requestErrMsg
                );
                promises.push(requestPromise);
            }
            requestIDsForGroup.push(requestIDsForType);
            const requestTypeErrMsg =
                "Attempted to seed type #" +
                typeIdx +
                " for group " +
                groupIdx +
                " but failed:";
            const requestTypePromise = createRequestType(
                typeIDsForGroup[typeIdx],
                requestIDsForType,
                groupIDs[groupIdx],
                requestTypeErrMsg
        ...
        const requestGroupErrMsg =
            "Attempted to seed group #" + groupIdx + " but failed:";
        const requestGroupPromise = createRequestGroup(
            groupIDs[groupIdx],
            requestGroupNames[groupIdx],
            typeIDsForGroup,
            requestGroupErrMsg
        );
    ...
    Promise.all(promises).then(() => {
        ...
        for (let groupIdx = 0; groupIdx < numGroups; groupIdx++) {
            for (let typeIdx = 0; typeIdx < numTypesPerGroup; typeIdx++) {
                for (
                    let requestIdx = 0;
                    requestIdx < requestIDs[groupIdx][typeIdx].length;
                    requestIdx++
                ) {
                    updatePromises.push(
                        Request.findByIdAndUpdate(
                            requestIDs[groupIdx][typeIdx][requestIdx],
                            { requestType: typeIDs[groupIdx][typeIdx] }
                        )
                    );
                }
                updatePromises.push(
                    RequestType.findByIdAndUpdate(typeIDs[groupIdx][typeIdx], {
                        requestGroup: groupIDs[groupIdx]
                    })
                );
            }
            updatePromises.push(
                RequestGroup.findByIdAndUpdate(groupIDs[groupIdx], {
                    requestTypes: typeIDs[groupIdx]
                })
            );
        }
